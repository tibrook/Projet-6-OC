...
export class SaucesService {
  ...
  getSauces() {
    this.http.get<Sauce[]>('https://localhost:3000/api/sauces').pipe(
  ...
  getSauceById(id: string) {
    return this.http.get<Sauce>('https://localhost:3000/api/sauces/' + id).pipe(
  ...
  likeSauce(id: string, like: boolean) {
    return this.http.post<{ message: string }>(
      'https://localhost:3000/api/sauces/' + id + '/like',
  ...
  createSauce(sauce: Sauce, image: File) {
    ...
    return this.http.post<{ message: string }>('https://localhost:3000/api/sauces', formData).pipe(
      catchError(error => throwError(error.error.message))
    );
  }
  ...
  modifySauce(id: string, sauce: Sauce, image: string | File) {
    if (typeof image === 'string') {
      ...
      formData.append('sauce', JSON.stringify(sauce));
      formData.append('image', image);
      return this.http.put<{ message: string }>('https://localhost:3000/api/sauces/' + id, formData).pipe(
        catchError(error => throwError(error.error.message))
      );
    }
  }
  ...
  deleteSauce(id: string) {
    return this.http.delete<{ message: string }>('https://localhost:3000/api/sauces/' + id).pipe(
      catchError(error => throwError(error.error.message))
    );
  }
}
